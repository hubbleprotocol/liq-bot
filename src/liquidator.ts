...
import {
  ...
  LiquidationsQueue,
  LiquidatorAssociatedTokenAccounts,
  UserMetadata,
  TokenPrices,
  CollateralTokenActive,
  CollateralAmounts,
  TokenMap,
  Loan,
  CollateralInfo,
...
export class Liquidator {
  ...
  constructor(
    ...
    this.getLiquidatorAssociatedTokenAccounts().then(
      async (liquidatorAssociatedTokenAccounts) => {
        this.liquidatorAssociatedTokenAccounts =
          liquidatorAssociatedTokenAccounts;
        ...
        Object.keys(CollateralTokenActive)
          .filter((token) => isNaN(parseInt(token)))
          .forEach(async (tokenAccount) => {
            this.clearLiquidationGainsIXMap.set(
              tokenAccount,
              await this.getClearLiquidationGainsIX(
                tokenAccount,
                new PublicKey(
                  this.liquidatorAssociatedTokenAccounts[tokenAccount]
                )
              )
            );
          });
    ...
    this.pollingAccountsFetcher = new PollingAccountsFetcher(
      process.env.RPC_URL,
      500,
      5
    );
    ...
    this.pollingAccountsFetcher.addProgram(
      'liquidationsQueue',
      clusterConfig.borrowing.accounts.liquidationsQueue,
      this.hubbleProgram,
      async (liquidationsQueue: LiquidationsQueue) => {
        this.liquidationsQueue = liquidationsQueue;
    ...
    this.pollingAccountsFetcher.addProgram(
      'borrowingMarketState',
      clusterConfig.borrowing.accounts.borrowingMarketState,
      this.hubbleProgram,
      async (borrowingMarketState: BorrowingMarketState) => {
        this.borrowingMarketState = borrowingMarketState;
    ...
    Object.keys(this.clusterConfig.borrowing.accounts.pyth)
      .filter((key) => key.includes('PriceInfo'))
      .forEach((key) => {
        // set prices to zero temporarily
        this.prices[key.split('Price')[0]] = {
          value: new Decimal(0),
          exp: new Decimal(0),
        };
        ...
        this.pollingAccountsFetcher.addConstructAccount(
          this.clusterConfig.borrowing.accounts.pyth[key],
          (data: Buffer) => {
          ...
          async (priceData: PriceData) => {
            ...
            if (priceData.price && priceData.exponent) {
              this.prices[key.split('Price')[0]] = {
                value: new Decimal(priceData.price),
                exp: new Decimal(priceData.exponent),
              };
              // calculate the minimum collateral ratio whenever the price changes
              this.mcr = await this.calculateMcr();
            } else {
              console.log(
                `Pyth Oracle: ${priceData.productAccountKey.toBase58()} - ${Object.keys(
                  this.clusterConfig.borrowing.accounts.pyth
                )
                  .map((key) => ({
                    key,
                    value: this.clusterConfig.borrowing.accounts.pyth[key],
                  }))
                  .find(
                    (keyValue) =>
                      keyValue.value === priceData.productAccountKey.toBase58()
                  )
                  .key.split('Pr')[0]
                  .toUpperCase()} - price ${PriceStatus[priceData.status]}`
              );
            }
          },
          (error) => {
  ...
  async getClearLiquidationGainsIX(
    token: string,
    clearerAassociatedTokenAccount: PublicKey
    ...
    const keys = [
      ...
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.borrowingMarketState
        ), // borrowingMarketState
        isWritable: false,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.globalConfig
        ), // globalConfig
        isWritable: false,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.borrowingVaults
        ), // borrowingVaults
        isWritable: false,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stabilityPoolState
        ), // stabilityPoolState
        isWritable: true,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stabilityVaults
        ), // stabilityVaults
        isWritable: false,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.liquidationsQueue
        ), // liquidationsQueue
        isWritable: true,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.collateralVault[token]
        ), // collateralVault
        isWritable: true,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.collateralVaultsAuthority
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts[
    ...
    return new TransactionInstruction({
      data: this.hubbleProgram.coder.instruction.encode(
  ...
  async getTryLiquidateIX(
    ...
    const keys = [
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.epochToScaleToSum
        ), //epochToScaleToSum
        isWritable: true,
        isSigner: false,
      },
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stablecoinMint
        ), // stablecoinMint
        isWritable: true,
        isSigner: false,
      },
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stablecoinMintAuthority
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stablecoinStabilityPoolVault
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.stablecoinStabilityPoolVaultAuthority
      ...
      {
        pubkey: new PublicKey(
          this.clusterConfig.borrowing.accounts.oracleMappings
        ), // oracleMappings
        isWritable: false,
        isSigner: false,
      },
  ...
  async clearLiquidationGains(): Promise<Array<string>> {
    const blockhash = (
      await this.hubbleProgram.provider.connection.getRecentBlockhash()
    ).blockhash;
    return flat(
      await Promise.all(
        Object.keys(CollateralTokenActive)
  ...
  async getPendingDebt(user: UserMetadata): Promise<Decimal> {
    const diffStableRpt = new Decimal(
      this.borrowingMarketState.stablecoinRewardPerToken.toString()
    ).minus(new Decimal(user.userStablecoinRewardPerToken.toString()));
    return user.status !== 1 || diffStableRpt.isZero()
      ? new Decimal(0)
      : diffStableRpt
          .mul(new Decimal(user.userStake.toString()))
          .dividedBy(DECIMAL_FACTOR);
  }
  ...
  async calculateTotalDebt(user: UserMetadata): Promise<Decimal> {
    ...
    return new Decimal(user.borrowedStablecoin.toString())
      .plus(pendingDebt)
      .dividedBy(new Decimal(STABLECOIN_DECIMALS));
  }

  // helper function, init a new CollateralAmounts object
  zeroCollateral(): CollateralAmounts {
    const keys = Object.keys(
      this.borrowingMarketState.collateralRewardPerToken
    ).filter(
      (key) =>
        !Array.isArray(this.borrowingMarketState.collateralRewardPerToken[key])
    );
  ...
  mulFrac(
    collateralAmmounts: CollateralAmounts,
    numerator: BN | Decimal,
    denominator: BN | Decimal
  ): TokenMap {
    Object.keys(collateralAmmounts)
      .filter((key) => Array.isArray(collateralAmmounts[key]))
      .forEach((key) => {
        collateralAmmounts[key] = collateralAmmounts[key]
  ...
  async getPendingCollateral(user: UserMetadata): Promise<CollateralAmounts> {
    ...
    Object.keys(this.borrowingMarketState.collateralRewardPerToken)
      .filter(
        (key) =>
          !Array.isArray(
            this.borrowingMarketState.collateralRewardPerToken[key]
          )
      )
      .forEach((key) => {
        diffCollRpt[key] = this.borrowingMarketState.collateralRewardPerToken[
          key
        ].sub(user.userCollateralRewardPerToken[key]);
      });
    return user.status !== 1 ||
      !Object.keys(diffCollRpt).some((key) => !diffCollRpt[key].isZero())
      ? (this.zeroCollateral() as TokenMap)
      : this.mulFrac(
          diffCollRpt,
          new Decimal(user.userStake.toString()),
          DECIMAL_FACTOR
        );
  }

  // add CollateralAmounts together
  async addCollateralAmounts(
    collateralAmounts: Array<CollateralAmounts>
  ): Promise<CollateralAmounts> {
    const keys = Object.keys(collateralAmounts[0]).filter(
      (key) => !Array.isArray(collateralAmounts[0][key])
    );
  ...
  precisionFromKey(key: string): Decimal {
    switch (key) {
      case 'sol':
        return new Decimal(LAMPORTS_PER_SOL);
      case 'btc':
        return new Decimal(DECIMALS_BTC);
      case 'msol':
        return new Decimal(LAMPORTS_PER_MSOL);
      case 'ray':
        return new Decimal(DECIMALS_RAY);
      case 'ftt':
        return new Decimal(DECIMALS_FTT);
      case 'eth':
        return new Decimal(DECIMALS_ETH);
      case 'srm':
        return new Decimal(DECIMALS_SRM);
    }
  ...
  lamportsToDecimal(collateralAmounts: CollateralAmounts): CollateralAmounts {
    ...
    Object.keys(collateralAmounts)
      .filter((key) => !Array.isArray(collateralAmounts[key]))
      .forEach((key) => {
        newCollateralAmounts[key] = new Decimal(
          collateralAmounts[key].toString()
        ).div(this.precisionFromKey(key));
      });
  ...
  async calculateTotalCollateral(
    user: UserMetadata
  ): Promise<CollateralAmounts> {
    return await this.addCollateralAmounts([
      this.lamportsToDecimal(user.depositedCollateral),
      this.lamportsToDecimal(user.inactiveCollateral),
      this.lamportsToDecimal(await this.getPendingCollateral(user)),
    ]);
  }
  ...
  async getLoans(metadatas: Array<UserMetadata>): Promise<Array<Loan>> {
    return (
      await Promise.all(
        metadatas
          .filter((metadata) =>
            new Decimal(metadata.borrowedStablecoin.toString()).gt(
              new Decimal(0)
            )
          )
          .map(async (metadata) => {
      ...
      return {
        ...loan,
        icr: Object.keys(loan.collateral)
          .filter((key) => !Array.isArray(loan.collateral[key]))
          .map((key) =>
            new Decimal(loan.collateral[key].toString()).mul(
              this.prices[key].value
            )
          )
          .reduce((a, b) => a.plus(b), new Decimal(0))
          .div(new Decimal(loan.usdhDebt.toString()))
          .mul(new Decimal(100)),
      } as Loan;
    });
  }
  ...
  async tryLiquidateUsers(): Promise<void> {
    ...
    const loans = await this.getLoans(
      [...this.pollingAccountsFetcher.accounts.values()]
        .filter((acc) => acc.accountKey === 'userMetadata')
        .map((acc) => acc.data as UserMetadata)
    );
    // loop through each loan and try to liquidate
    const sortedLoans = loans.sort(
      (loanA, loanB) => loanA.icr.toNumber() - loanB.icr.toNumber()
    );
    ...
    sortedLoans.forEach(async (loan) => {
      ...
      if (!(loan.icr as Decimal).eq(new Decimal(0))) {
        ...
        if (liquidatable) {
          console.log(
            `${loan.metadata.metadataPk.toBase58()} has ltv ${(
              ltv * 100
            ).toFixed(2)} attempting to liquidate`
          );
          let tx = new Transaction();
          tx = tx.add(
            this.liquidationIXMap.get(loan.metadata.metadataPk.toBase58())
          );
          tx.recentBlockhash = (
            await this.hubbleProgram.provider.connection.getRecentBlockhash()
          ).blockhash;
          ...
          console.log(
            'https://solscan.io/tx/' +
              signature +
              (this.cluster === 'devnet' ? '?cluster=devnet' : '')
          );
        }
      }
    });
  ...
  async getUserMetadatas(): Promise<void> {
    ...
    const userMetadataProgramAccounts =
      (await this.hubbleProgram.account.userMetadata.all()) as Array<
        ProgramAccount<UserMetadata>
      >;

    // loop through all the metadata and load new ones
    let usersLoaded = 0;
    await Promise.all(
      userMetadataProgramAccounts.map(async (programAccount) => {
        if (
          !this.pollingAccountsFetcher.accounts.has(
            programAccount.account.metadataPk.toBase58()
          ) &&
          programAccount.account.borrowingMarketState.toBase58() ===
            this.clusterConfig.borrowing.accounts.borrowingMarketState
        ) {
          ...
          this.pollingAccountsFetcher.addProgram(
            ...
            (userMetadata: UserMetadata) => {
              ...
              if (
                userMetadata.borrowingMarketState.toBase58() !==
                this.clusterConfig.borrowing.accounts.borrowingMarketState
              ) {
                console.log(
                  `removing ${userMetadata.metadataPk.toBase58()} from the polling account fetcher because of difference in market state key`
                );
                this.pollingAccountsFetcher.accounts.delete(
                  userMetadata.metadataPk.toBase58()
                );
              }
            },
            (error: any) => {
          ...
          this.liquidationIXMap.set(
            programAccount.account.metadataPk.toBase58(),
            await this.getTryLiquidateIX(programAccount.account.metadataPk)
          );
  ...
  async totalDepositedCollateral(): Promise<BN | Decimal> {
    return Object.keys(this.borrowingMarketState.depositedCollateral)
      .filter(
        (key) =>
          !Array.isArray(this.borrowingMarketState.depositedCollateral[key])
      )
      .map((key) => {
        return new Decimal(
          this.borrowingMarketState.depositedCollateral[key].toString()
        )
          .div(this.precisionFromKey(key))
          .mul(this.prices[key].value);
      })
      .reduce((a, b) => a.plus(b), new Decimal(0));
  }
  ...
  async totalCollateralRatio(): Promise<CollateralInfo> {
    return {
      collateralRatio: ((await this.totalDepositedCollateral()) as Decimal).div(
        new Decimal(
          this.borrowingMarketState.stablecoinBorrowed.toString()
  ...
  async calculateSystemMode(): Promise<[SystemMode, BN | Decimal]> {
    const tcr = (
      (await this.totalCollateralRatio()).collateralRatio as Decimal
    ).mul(new Decimal(100));
  ...
  getLiquidatorAssociatedTokenAccounts(): Promise<LiquidatorAssociatedTokenAccounts> {
    return new Promise((resolve) => {
      const liquidatorAssociatedTokenAccounts =
        {} as LiquidatorAssociatedTokenAccounts;
      const chunkOf5Tokens = chunk(
        Object.keys(this.clusterConfig.borrowing.accounts.mint),
        5
      ) as Array<Array<string>>;
      Promise.all(
        chunkOf5Tokens.map(async (chunkedTokens, chunkIndex) => {
          return await Promise.all(
            chunkedTokens.map((token) => {
              return new Promise((resolve) => {
                setTimeout(async () => {
                  resolve({
                    token,
                    pub: (
                      await getOrCreateAssociatedTokenAccount(
                        this.hubbleProgram.provider.connection,
                        this.wallet.payer,
                        new PublicKey(
                          this.clusterConfig.borrowing.accounts.mint[token]
                        ),
                        this.hubbleProgram.provider.wallet.publicKey
                      )
                    ).address,
                  });
                }, 1000 * chunkIndex);
        ...
        flat(tokenAddresses).forEach(
          (ata: { token: string; pub: PublicKey }) => {
            // mint uses WSOL for the key, while ActiveCollateralToken uses SOL ...
            liquidatorAssociatedTokenAccounts[
              ata.token === 'WSOL' ? 'SOL' : ata.token
            ] = ata.pub.toBase58();
          }
        );
  ...
  static async load(idl: Idl, configs: Array<Config>): Promise<Liquidator> {
    ...
    const connection = await TpuConnection.load(process.env.RPC_URL, {
    ...
    const hubbleProgram = new Program(
      idl as Idl,
      new PublicKey(clusterConfig.borrowing.programId),
      new Provider(connection, wallet, { commitment: 'processed' })
    );

    // load borrowing market state
    const borrowingMarketState =
      (await hubbleProgram.account.borrowingMarketState.fetch(
        clusterConfig.borrowing.accounts.borrowingMarketState
      )) as BorrowingMarketState;
    // load liquidations queue
    const liquidationsQueue =
      (await hubbleProgram.account.liquidationsQueue.fetch(
        clusterConfig.borrowing.accounts.liquidationsQueue
      )) as LiquidationsQueue;
