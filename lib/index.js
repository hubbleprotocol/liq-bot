...
class Bot {
    ...
    static async create() {
        ...
        const liquidator = await Liquidator.load(process.env.RPC_URL, process.env.CLUSTER, idl, configs);
    ...
    async start() {
        ...
        this.intervals.push(setInterval(async function () {
            ...
            if (JSON.stringify(bot.liquidator.idl) !== JSON.stringify(idl) && JSON.stringify(bot.liquidator.configs) !== JSON.stringify(configs)) {
                bot.stop();
                bot.liquidator = await Liquidator.load(process.env.RPC_URL, process.env.CLUSTER, idl, configs);
                bot.start();
            }
        }.bind(this), 60 * 1000));
...
class Liquidator {
    ...
    async getClearLiquidationGainsIX(token, clearerAassociatedTokenAccount) {
        ...
        const keys = [
            ...
            {
                pubkey: new web3_js_1.PublicKey(this.clusterConfig.borrowing.accounts.collateralVault[types_1.CollateralTokenActive[token][0] + types_1.CollateralTokenActive[token].substring(1).toLowerCase()]),
                isWritable: true,
                isSigner: false
            },
            {
            ...
            {
                pubkey: new web3_js_1.PublicKey(this.clusterConfig.borrowing.accounts['liquidationRewardsVault' + types_1.CollateralTokenActive[token][0] + types_1.CollateralTokenActive[token].substring(1).toLowerCase()]),
                isWritable: false,
                isSigner: false
            },
            {
    ...
    async getTryLiquidateIX(userMetadata) {
        ...
        ].concat(...Object.keys(types_1.CollateralTokenActive).filter(token => isNaN(types_1.CollateralTokenActive[token])).map(token => {
            return {
                pubkey: new web3_js_1.PublicKey(this.clusterConfig.borrowing.accounts.pyth[types_1.CollateralTokenActive[token].toLowerCase() + 'PriceInfo']),
                isWritable: false,
                isSigner: false
            };
        })).concat(...[
    ...
    async clearLiquidationGains() {
        const chunkOf5Tokens = chunk(Object.keys(types_1.CollateralTokenActive).filter(token => isNaN(types_1.CollateralTokenActive[token])), 5);
            ...
            return new Promise((resolve) => {
                setTimeout(() => {
                    ...
                    chunkedTokenAccounts.forEach(async (tokenAccount) => {
                        tx.add(await this.getClearLiquidationGainsIX(types_1.CollateralTokenActive[tokenAccount], new web3_js_1.PublicKey(this.liquidatorAssociatedTokenAccounts[types_1.CollateralTokenActive[tokenAccount]])));
                    });
    ...
    async tryLiquidateUsers() {
        ...
        loans.sort((loanA, loanB) => loanA.tcr.toNumber() - loanB.tcr.toNumber()).forEach(async (loan) => {
            if (!loan.tcr.eq(new decimal_js_1.default(0)) && loan.tcr.lt(this.mcr.plus(new decimal_js_1.default(10 * 0.0001)))) {
    ...
    static async load(RPC_URL, cluster, idl, configs) {
        ...
        const connection = await tpuClient_1.TpuConnection.load(RPC_URL, { commitment: 'processed' });
        ...
        return new Liquidator(cluster, idl, configs, clusterConfig, wallet, hubbleProgram, borrowingMarketState, liquidationsQueue, blockhash);
    }
}
